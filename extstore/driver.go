...
type ExternalStorageDriver interface {
	...
	Store(ctx ExternalStorageDriverContext, payloads []*commonpb.Payload) ([]ExternalStorageClaim, error)

	// Retrieve retrieves a list of payloads from external storage that are associated
	// with the provided list of storage claims.
	Retrieve(ctx ExternalStorageDriverContext, claims []ExternalStorageClaim) ([]*commonpb.Payload, error)
}
