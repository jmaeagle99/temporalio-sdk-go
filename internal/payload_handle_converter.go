...
func (dc *PayloadHandleDataConverter) FromPayload(payload *commonpb.Payload, valuePtr interface{}) error {
	...
	if ok {
		metadataPayload := &commonpb.Payload{}
		if metadataBytes, ok := payload.Metadata["userMetadata"]; ok {
			err := proto.Unmarshal(metadataBytes, metadataPayload)
			if err != nil {
				return err
			}
		}
		payloadHandle.Initialize(dc.innerConverter, payload, metadataPayload)
		return nil
	}
