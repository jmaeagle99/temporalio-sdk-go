...
func ensureRequiredParams(params *workerExecutionParameters) {
	...
	if params.DataConverter == nil {
		params.DataConverter, params.SetPayloadErrorLimits = newPayloadProcessingDataConverter(
...
func (aw *WorkflowReplayer) replayWorkflowHistoryRoot(
	...
	dataConverter, errorLimitsCallback := newPayloadProcessingDataConverter(
		newExternalStorageDataConverter(
			innerConverter,
			logger,
			extstore.ExternalStorageOptions{}),
		logger,
		PayloadLimitOptions{})
...
func NewAggregatedWorker(client *WorkflowClient, taskQueue string, options WorkerOptions) *AggregatedWorker {
	...
	innerConverter := client.originalDataConverter
	if client.externalStorage != nil {
		innerConverter = newExternalStorageDataConverter(
			innerConverter,
			logger,
			*client.externalStorage)
	}
...
func setClientDefaults(client *WorkflowClient) {
	if client.originalDataConverter == nil {
		...
		client.dataConverter, _ = newPayloadProcessingDataConverter(
			innerConverter,
			client.logger,
			client.payloadLimits)
	}
