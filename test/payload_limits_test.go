...
func (ts *PayloadLimitsTestSuite) TestMemoSizeWarningStartWorkflow() {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	wfName := "memo-size-warning-start-workflow"
	ts.worker.RegisterWorkflowWithOptions(
		func(ctx workflow.Context) error { return nil },
		workflow.RegisterOptions{Name: wfName},
	)

	// Create a memo whose aggregate size exceeds the warning threshold
	memoValue := strings.Repeat("m", 256)
	...
	run, err := ts.client.ExecuteWorkflow(ctx, opts, wfName)
	ts.NoError(err)
...
func (ts *PayloadLimitsTestSuite) TestMemoSizeErrorUpsertMemo() {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	...
	wfName := "memo-size-error-upsert-memo"
	ts.worker.RegisterWorkflowWithOptions(
		func(ctx workflow.Context) error {
			// Upsert a memo whose aggregate size exceeds the memo error limit.
			memoValue := strings.Repeat("a", MEMO_SIZE_ERROR_LIMIT/3)
